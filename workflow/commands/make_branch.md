...
Create or switch the product repo onto a git branch named from a ticket or plan filename. Use this before implementing work so product-repo branches stay aligned with `.documents/.tickets/...` or `.documents/.plans/...`.
...
- Examples:
  - `gus/feat/plan-1015-persist-wizard-state-2026-03-18`
  - `gus/fix/ticket-2044-fix-snapshot-refresh-2026-03-18`
...
5) **Create or switch branch in the product repo**
   - Run `./scripts/public-plan-branch <artifact-path> <type>`
   - This targets `../your-product-repo`, not the workflow repo.
6) **Report**  
   - Output the new branch name and product repo path.
   - If the product repo has unstaged or untracked files and cannot switch safely, stop and ask the user to clean or stash first.

## Guardrails
- Do not delete or rename existing branches.
- Do not modify repo files; this command only creates or switches a branch in the product repo.
