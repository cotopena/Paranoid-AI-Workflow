...
## Before You Run
- Read `workflow/README.md` to confirm the issue -> ticket -> research -> plan workflow.
- Inspect `workflow/tooling.config.json` for the `.documents/issues/` root and available read/search fallbacks.
- Example Codex CLI prompt: `Use workflow/commands/mirror_issue.md to mirror Linear issue PROJ-42 about wizard step persistence into .documents/issues/current/.`
...
- Filename: `<ISSUEKEY>-short-slug-YYYY-MM-DD.md`
  - Keep the issue key exactly as provided (`PROJ-42`, `ENG-103`, etc.)
